...
for source_name, url in sources.items():
    ...
    try:
        ...
        # Add this line to see how many articles were found
        print(f"  Found {len(paper.articles)} article URLs")
    ...
    for art in paper.articles:
        attempted += 1  # Track every attempt
        ...
        try:
            ...
            if len(art.text.strip()) > 100:
                ...
                count += 1
                successful += 1  # Track successful articles
            else:
                too_short += 1  # Track articles that were too short
                print(f"  -> Article too short: {art.url}")
                count += 1  # IMPORTANT: Still increment count to match your original logic

        except (ArticleException, ArticleBinaryDataException) as e:
            failed_download += 1  # Track download failures
            print(f"  -> Download failed: {art.url} - {str(e)[:50]}")
            count += 1  # Still increment count for failed articles
            continue
        except Exception as e:
            failed_parse += 1  # Track parsing failures
            print(f"  -> Skipped article: {str(e)[:50]}")
            count += 1  # Still increment count
            continue

    # Print detailed statistics for this source
    print(f"\n  {source_name} Statistics:")
    print(f"    Total URLs found: {len(paper.articles)}")
    print(f"    Attempted to process: {attempted}")
    print(f"    Successfully processed: {successful}")
    print(f"    Too short (<100 chars): {too_short}")
    print(f"    Failed to download: {failed_download}")
    print(f"    Failed to parse: {failed_parse}")
    print(f"    Hit MAX_ARTICLES limit: {attempted >= MAX_ARTICLES}")
    print(f"    Count variable reached: {count}")

    print(f"\nTotal articles collected across all sources: {len(articles)}")
